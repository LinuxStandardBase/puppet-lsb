...
cd $MAINT_SCRIPTS_HOME/manifest-old && ./manifest_rebuild_s.sh >cronmsg 2>&1
grep -i '\(warning\|error\)' cronmsg >/dev/null 2>&1 && echo -e "\n\n--- Manifest-old rebuild ---\n\n" && cat cronmsg

cd $MAINT_SCRIPTS_HOME/manifest && ./manifest_rebuild_s.sh >cronmsg 2>&1
grep -i '\(warning\|error\)' cronmsg >/dev/null 2>&1 && echo -e "\n\n--- Manifest rebuild ---\n\n" && cat cronmsg

#cd $MAINT_SCRIPTS_HOME/manifest && ./manifest_rebuild_moblin.sh >cronmsg 2>&1
#grep -i '\(warning\|error\)' cronmsg >/dev/null 2>&1 && echo -e "\n\n--- Manifest for Moblin ---\n\n" && cat cronmsg
